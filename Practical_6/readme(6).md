...
> **Where:** `n` = Number of matrices in the chain
...
* The `p` list stores the dimensions of the matrices and requires **O(n)** additional space.
...
The algorithm has **O(n³)** time complexity in the **best, average, and worst cases**, because it checks all possible chain lengths, starting positions, and split positions. Its space complexity is **O(n²)** because of the 2D DP table.

This Dynamic Programming approach avoids repeated calculations by **storing previously calculated results**. It efficiently finds the **minimum number of scalar multiplications** required to multiply the given chain of matrices.
